let searcharrivalsid
let mindepo
async function searcharrivalsActive() {
    const form = document.querySelector('#searcharrivalsform')
    const form2 = document.querySelector('#searcharrivalscheckinsform')
...
function openarrivaltab(where){
    if(where == 'checkin' && searcharrivalsid){
        did('searcharrivalsformcontainer').classList.add('hidden')
        did('searcharrivalscheckinsform').classList.remove('hidden')
    }
    if(where == 'arrivals'){
        did('searcharrivalsformcontainer').classList.remove('hidden')
        did('searcharrivalscheckinsform').classList.add('hidden')
    }
}
    ...
    if(request.status) {
        if(!id){
            if(request.data.length) {
                ...
                resolvePagination(datasource, onsearcharrivalsTableDataSignal)
            }
        }else{
             searcharrivalsid = request.data[0].id
...
async function removesearcharrivals(id) {
    // Ask for confirmation
    const confirmed = window.confirm("Are you sure you want to remove this searcharrivals?");
...
async function onsearcharrivalsTableDataSignal() {
    ...
    <tr> 
        ...
        <td>
            <div class="w-full h-full flex items-center justify-center gap-4">
                <button onclick="opencheckinreceiptsearcharrivals('${item.reservations.id}', '${r}', '${x}')" class="material-symbols-outlined rounded-full bg-gray-100 h-8 w-8 text-green-400 drop-shadow-md text-xs" style="font-size: 18px;">visibility</button>
                <button title="check in" onclick="did('reservationcheckin').click();sessionStorage.setItem('checkinfromsomewhere', '${item.reservations.id}')" class="material-symbols-outlined ${item.reservations.status != 'CHECKED IN' ? '' : 'hidden'} rounded-full bg-green-400 h-8 w-8 text-white drop-shadow-md text-xs" style="font-size: 18px;">check_circle</button>
...
function opencheckinreceiptsearcharrivals(id, ratee, rooms){
...
function fetchguestsarrivalreservations(id){
    ...
    did('mindeposit1').innerHTML = `(minimum deposit is ${formatCurrency(rumcat.filter(data=>data.id == datfilt.reservations.roomcategory)[0].minimumrequireddeposit)})`
    mindepo = Number(rumcat.filter(data=>data.id == datfilt.reservations.roomcategory)[0].minimumrequireddeposit)
    hemsroomnumber(datfilt.reservations.roomcategory)
}

async function searcharrivalsFormSubmitHandler(id) {
    if(!validateForm('searcharrivalsform', getIdFromCls('comp'))) return
...
async function searcharrivalscheckinFormSubmitHandler(id) {
    if(!validateForm('searcharrivalscheckinsform', getIdFromCls('comp'))) return
    // if(Number(mindepo) <= Number(document.getElementById('amountpaid').value))return notification(`Amount paid must be more than or equal to ${formatCurrency(mindepo)}`, 0)
    
    let payload

    payload = getFormData2(document.querySelector('#searcharrivalscheckinsform'), null)
    let request = await httpRequest2('../controllers/checkin', payload, document.querySelector('#searcharrivalscheckinsform #submit'))
    // if(!id)document.getElementById('tabledata').innerHTML = `No records retrieved`
    if(request.status) {
        notification('Checked in Successfully', 1)
                did('searcharrivals').click()
    }
    else return notification('No records retrieved')
}


// function runAdsearcharrivalsFormValidations() {
//     let form = document.getElementById('searcharrivalsform')
//     let errorElements = form.querySelectorAll('.control-error')
//     let controls = []

//     if(controlHasValue(form, '#owner'))  controls.push([form.querySelector('#owner'), 'Select an owner'])
//     if(controlHasValue(form, '#searcharrivalsname'))  controls.push([form.querySelector('#searcharrivalsname'), 'searcharrivals name is required'])
