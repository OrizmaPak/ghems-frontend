...
| `pp_approvepersonnel` | `approvepersonnel.php`, `js/approvepersonnel.js` | `fetchpersonnels.php`, `personnelapprovals.php` | Implemented with dedicated HEMS renderer. Load only `status=NOT APPROVED`. Batch approve/decline sends `buttonselected`, `ids0..idsN`, `idsize`. Row actions view the full personnel modal or open Add Personnel for edit. Do not show department/group filters or columns. |
