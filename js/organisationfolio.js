...
function getOrganisationFolioPrintTitle() {
    if(isCompanyFolioRoute()) return 'COMPANY FOLIO'
    if(isAgencyFolioRoute()) return 'AGENCY FOLIO'
    if(isGroupFolioRoute()) return 'GROUP FOLIO'
    return 'FOLIO'
}
...
function getGuestFolioPrintModel(guestId = '') {
    ...
    transactions.forEach((tx) => {
        ...
        const effectiveCredit = isGuestFolioRoute() && tx?.hasHiddenCredit ? 0 : credit
        totalDebit += debit
    ...
    const rows = Array.from(guestFolioBuckets.values()).map((bucket, index) => {
        ...
        return `
            <tr>
                ...
                <td><button onclick="orgFolioOpenPrint('${bucket.folioid || bucket.guestid}')" class="btn btn-sm bg-slate-700 text-white">View/Print Folio</button></td>
            </tr>
        `
    }).filter(Boolean).join('')
...
function openGuestFolioPrint(guestId = '') {
    ...
    const folioTitle = getOrganisationFolioPrintTitle()
    ...
    invoiceContainer.innerHTML = `
        <div class="flex min-h-0 flex-1 flex-col bg-white">
            ...
            <div class="header">
                ...
                <div class="title">${folioTitle}</div>
            </div>
            ...
            <div class="flex shrink-0 justify-end gap-2 border-t border-slate-200 bg-white/95 p-3">
                <button type="button" class="btn" onclick="printContent('HEMS ${folioTitle}', null, 'guestfolioprintcontainer', true)">Print</button>
    ...
    did('receivablesRoomPickerModal').innerHTML = `
      <div class="max-w-5xl w-full bg-white rounded shadow p-4 max-h-[90vh] overflow-auto">
        ...
        <div class="flex flex-wrap gap-2 mb-3 items-end">
          <button type="button" id="receivablesPickerTabCheckedin" class="inline-block p-3 border-b-2 border-blue-500 text-blue-600 font-semibold" onclick="orgFolioSwitchPickerTab('checkedin')">All Checked In</button>
          <button type="button" id="receivablesPickerTabReservations" class="inline-block p-3 border-b-2 border-transparent text-gray-500 font-semibold" onclick="orgFolioSwitchPickerTab('reservations')">All Reservations</button>
          <div class="ml-auto grid grid-cols-1 md:grid-cols-3 gap-2 w-full md:w-auto">
            ...
            <button type="button" class="btn btn-sm" onclick="orgFolioReloadPickerTabData()">Filter</button>
          </div>
...
function renderReceivablesPickerRows(){
    ...
    did('receivablesPickerRows').innerHTML = rows.map((item, idx) => `
        <tr>
            ...
            <td><button type="button" class="btn btn-sm bg-blue-500 text-white" onclick='orgFolioUseRoomPicker(${idx})'>Use</button></td>
        </tr>
...
async function onreceiveablesTableDataSignal() {
    ...
    if(guestfolioReceiveablesFiltered || isPayPendingCheckoutBillsRoute()){
        ...
        <tr>
            ...
            <td><p class="text-black font-semibold">${formatFolioAmountCell(runningBalance)}</p></td>
            <td><button onclick="orgFolioOpenReceivableModalByIndex('${item.index ?? index}')" class="btn btn-sm btn-primary ${result > 0 ? '' : '!hidden'}">Pay Now</button></td>
        </tr>`)}
        )
        .join('')
    ...
    did('invoicecontainer').innerHTML = `
        <div class="rounded-lg w-[640px] max-w-full">
            ...
            <div class="flex items-center justify-end gap-3">
                ...
                <button id="receivable-pay-submit" type="button" class="btn" onclick="orgFolioSubmitReceivablePayment()">Pay Now</button>
            </div>
        </div>
    `
...
window.companyfolioActive = companyfolioActive
window.agencyfolioActive = agencyfolioActive
window.groupfolioActive = groupfolioActive
window.orgCompanyfolioActive = companyfolioActive
window.orgAgencyfolioActive = agencyfolioActive
window.orgGroupfolioActive = groupfolioActive
window.orgFolioOpenPrint = openGuestFolioPrint
window.orgFolioSwitchPickerTab = switchReceivablesPickerTab
window.orgFolioReloadPickerTabData = reloadReceivablesPickerTabData
window.orgFolioUseRoomPicker = useReceivablesRoomPicker
window.orgFolioOpenReceivableModalByIndex = openreceiveablemodalbyindex
window.orgFolioSubmitReceivablePayment = submitReceivablePayment
})();
